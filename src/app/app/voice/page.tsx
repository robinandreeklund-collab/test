...
export default function Voice() {
  ...
  useEffect(() => {
    ...
    r.onerror = (e: any) => {
      ...
      const map: Record<string, string> = {
        ...
        'service-not-allowed': 'Microphone access was blocked. Allow the mic for this site and try again.',
        'no-speech': "I didn't catch that — tap and speak again, or type below.",
        'audio-capture': 'No microphone was found. You can type your question below.',
        network: 'Voice needs a connection to transcribe — you can type instead.',
      };
      setNotice(map[e?.error] ?? 'Voice input stopped. You can type your question below.');
    };
    recogRef.current = r;
  ...
  function toggle() {
    ...
    setReply(''); setTranscript(''); setNotice('');
    try { r.start(); setListening(true); trackClient('voice_listen'); }
    catch { setNotice('Could not start the microphone. You can type your question below.'); }
  }
  ...
  return (
    <div className="screen center">
      ...
      {notice && (
        <div className="banner warn" style={{ marginTop: 10, textAlign: 'left', wordBreak: 'break-word' }}>
          {notice}
        </div>
      )}
